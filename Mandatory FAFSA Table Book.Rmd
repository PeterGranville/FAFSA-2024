...
preprocess.t1 <- function(data0, stateName, year0, year1){
  ...
  data0 <- data0 %>% arrange(`State Category`)
...
studentTable <- studentTable %>% add_row(
  ...
  `Selected students` = "Two or more races undergraduates"
)
studentTable <- studentTable %>% add_row(
  `Variable` = "All",
...
preprocessEnroll <- function(data0){
  ...
  data0 <- data0 %>% rename(
    `Comparison years` = `Comparison years (Fall semester)`
  ) %>% select(
    `Selected state`, 
...
Tables E1 through E5 inform Table 5 in the report. 
...
```{r mixedStatus}
kable(aggMS) %>% kable_paper("hover", full_width = F)
```
